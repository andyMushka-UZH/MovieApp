...
class RaingView: UIView {
    ...
    private func prepareBackgoundView() {
        backgoundView.layer.cornerRadius = bounds.width / 2
    ...
    private func addCircle(layer: CAShapeLayer, color: UIColor, opacity: Float) {
        ...
        let center = CGPoint(x: bounds.origin.x + (bounds.width / 2),
                             y: bounds.origin.y + (bounds.height / 2))
        
        layer.path = UIBezierPath(arcCenter: center,
                                  radius: (frame.height / 2) - calculateScaling(constant: 10),
                                  startAngle: -.pi/2,
                                  endAngle: (.pi/2) * 3 ,
    ...
    private func calculateScaling(constant: CGFloat) -> CGFloat {
        return constant * (bounds.height / kWidthForScale)
    }
